...
if GOOGLE_CREDS_JSON:
    ...
    app.logger.warning("Variável GOOGLE_CREDS_JSON não definida")
    GOOGLE_CREDS = None

# ID da planilha (CORRIGIDO: usar variável correta)
SHEET_KEY = os.environ.get('JCM_SHEET_ID', '1pKPQYEzI8SxuLG8wvZGQDOz4q8Tar5Oj')  # ID da planilha
...
# Inicialização Google Sheets (CORRIGIDO)
...
# ================= ROTAS ESSENCIAIS (CORRIGIDAS) =================
@app.route('/')  # ROTA RAIZ ADICIONADA
def home():
    return "Aline Bot está online! ✅"

@app.route('/healthz', methods=['GET', 'HEAD'])  # CORRIGIDO: '/healthz' com 'z'
def health_check():
...
# ================= SERVIDOR PRODUÇÃO (CORRIGIDO) =================
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    
    # CORREÇÃO: Verificar ambiente corretamente
    if os.environ.get('FLASK_ENV') == 'production' or os.environ.get('ENV') == 'production':
        ...
        app.run(host="0.0.0.0", port=port, debug=True)  # Debug ativado para desenvolvimento
